...
class MenuDvd():
    ...
    def __init__(self, titulo, lst_item, lst_func=None, fraseHead=''):               
        ...
        """ 
        Mensaje Unico pedir dato al usuario """
        self.introData = '\nIntro Opt (XindeX)..... '     
    ...
    def style(self, char_head= CHAR_HEAD, 
        ...
        self.corona, self.cabeza, self.cuello, self.pie = self.formar_entorno()
    # ___________________
    # Retorna 4 espacios
    def get_TAB(self):
        return self.__TAB
    ...
    def formar_entorno(self):
        ...
        return corona, cabeza, cuello , pie
    ...
    def F_r_a_n_k_Y(self, bSombrero=False, bCabeza=False, bCuello=False, bSalir=False ,  bCuerpo=False, bPie=False, esNumerado=False):
        lst_franky=[]
        retorno = ''
        if bSombrero:   lst_franky.append( f'{self.corona}' )
...
class XindeX(MenuDvd):
    ...
    def __init__(self, esLoop=True):
        ...
        self.lst_titulosXX=[]
        """ >>> Lista de TODOS LOS titulos de menu introducidos. Me permite validar rapido  """
    ...
    # 3-xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
    ...
    def Mystyca(self, titulo, configurado=False, execFunc=True, tipo_marcador=0, execAll=True, Loop=True, padX=30):
        ...
        if configurado == True:        #  PRINT SUB-MENUS 
            ...
            """ >>> GENERA LA  M A T R I Z   D E   I M P R E S I O N  CON NUMERACION RELATIVA 
            """
            ...
            """ >>> LISTA DE List de Str(Matriz) de las O P C I O N E S    V A L I D A S  
    ...
    def Mystyca_Skin(self, menu_dvd, level=None, retorno=None):
        """ >>> Crea una matriz con las lineas str del menu en forma de lista.... pero la numeracion es la relativa(la de MenuDvd) 
        Crea la recursividad dependiendo de si tiene hijos o no tiene hijos, de esta manera se relaciona
        la configuracion de la genetica con las lineas de impresion.
        """
        if level==None and retorno==None:     # 1ª ENTRADA
            level=-1 ;  retorno=[] ; self.pasos=0 
        level += 1
        ...
        if  lst_hijos:         
            ...
            for i, item in enumerate(menu_dvd.lst_item):
                retorno.append(menu_dvd.get_lst_row_body(row=i))
                """ >>> mete una lst que represeta una linea del menu.
                """
                for hijo in lst_hijos:
                    if hijo['ind_en_padre'] == i:
                        # ╔╦╦╔╦╦╔╦╦╔╦╦╔╦╦╔╦╦╔╦╦╔╦╦╔╦╦╔╦╦╔╦╦╔╦╦╔╦╦╔╦╦╔╦╦╔╦╦╔╦╦╔╦╦╔╦╦╔╦╦╔╦╦╔╦╦╔╦╦╔╦╦
        ...
        while(True):
            ...
            try:
                if respuesta == SALIR:          # A PULSADO  '<<<' ....SALIR
                    ...
                    """  M O D O   B a c k G r o u n d :    Acaba cuando yo me acabe (demonio) """
                    ...
                    """ M O D O   I N T E R A C T I V O """
                    """ interactivo (no demonio)(tkinter o si quieres que siga la ejecución aunque yo me acabe) """
                    respuesta_extraida = respuesta[2:].strip()   #Desde la posicion 2 hasta el final . respuesta!!.
                    pre_respuesta = respuesta[:2].strip()   # Desde el inicio(0) hasta 1(2-1) >>
                
                elif respuesta.startswith('*'):             
                    """ M O D O   D I R E C T O R I O    W I P """
                    if len(respuesta) < 1: 
                        continue 
                    ...
                    if respuesta in lista:
                        for i, str_indice in enumerate(lista):
                            if respuesta == str_indice:
                                ...
                                """ 
                                >>> MODO DE OPCION VALIDA(TODO vale. | solo HIJOS | Modo DIRECTORIO->Lanza Sub-Menus ) """
                                if execAll == None: execAll = True
                                if isinstance(execAll, bool):
                                    if execAll == True:     # Todo son opciones validas.... responsabilidad del usuario meter funciones en directorios.
                                        ...
                                        pass
                                elif isinstance(execAll,str):
                                    if execAll == 'D':
                                        pass
                                        """ modo directorio. Hay que poder mostrar cachos de Mystyka_Skin """
                                    else:
                                        pass
                                        """ otros modos para futuro, de momento solo D """
                                    pass            
                                else:
                                    execAll = True
                                pass                               
    ...
    def create_INDICEX(self, lst__men_itm_lev_ape_name_padr_ipadr ): 
        ...
        return lst_opt_tipo_NUMERIC , lst_opt_tipo_ALF_MAX , lst_opt_tipo_ALF_MIN , lst_opt_tipo_MIXTO
...
class Over_Main(XindeX):
    ...
    # S o b r e e s c r i b e  Terminator de XindeX (para ampliar su funcionalidad) ... pero le vuelve a llamar con super
