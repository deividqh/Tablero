...
class listTOdict_byTcld():
    ...
    """
    TIPOS_VALIDOS = {
        int: lambda v: int(v) if v.isdigit() else None,
        float: lambda v: float(v) if re.match(r"^-?\d*\.\d+$", v) else None,
        str: lambda v: v,
        bool: lambda v: v.lower() in ["true", "1", "yes", "y"],
        list: lambda v: v.split(","),
        set: lambda v: set(v.split(",")),
        ...
        "between": lambda v, opciones: v if v in opciones else opciones[0] if None in opciones else None
    }
    def __init__(self):
        """ 
        Constructor: 
    ...
    def __validar_tipo(valor, tipo, opciones=None):
        ...
        """
        if tipo in listTOdict_byTcld.TIPOS_VALIDOS:
            if tipo != "between":
                return listTOdict_byTcld.TIPOS_VALIDOS[tipo](valor)  
            else: 
                listTOdict_byTcld.TIPOS_VALIDOS[tipo](valor, opciones)
        return valor
    ...
    # ********************
    @staticmethod
    def __introByDef(valor, options:dict=None):
        """ 
        Llamada desde byDef. 
        ...
        # _____________________
        # if esCapital:
            # valorLista = str(valor.group()).capitalize()
        # else:
        valor_lista = str(valor.group())
        # _____________________
        while True:
            retorno = input(f'{msgIntro}-{valor_lista} ..... ')
    ...
    def byDef(listaStrKeys, listaDef=None, permiteNulo=True, msgIntro='Intro'):
        """         
        ...
        [permiteNulo] => bool. =True Permite Nulo en Entrada por teclado....lo capa por encima de listDef
        ...
        dictRetorno = {
            ...
            strKey:re.sub(patron, lambda match: listTOdict_byTcld.__introByDef(match, options), listaStrKeys)
        }
        # _____________________
    ...
    def __validaListasEntrada(listaKeys, listaDef):
        """  
    ...
    def __tiparDiccionario(diccionario, listaDef):
        ...
        # _________________
        # IGUALA LA LONGITUD DE LAS LISTAS (cambia la longitud de listaDef)
        ...
        for i, valor in enumerate(lst_valores):            
            ...
            if listaDef[i][TIPO]==int:
                try:
                    ...
                    # listTOdict_byTcld.__excepcionTipado(valor=valor, lista=lst_valores_tipados, permiteNulo=listaDef[i][PERMITENULL], tipo=listaDef[i][TIPO])
                    listTOdict_byTcld.__setValoresByDef(lista=lst_valores_tipados, tipo=listaDef[i][TIPO])
            # ________________
            ...
            # ________________
                ...
                try:
                    ...
                    # lst_valores_tipados.append(bool(valor))
        ...
        # print(lst_valores_tipados)
        pass
        # Ahora tengo una lista con los valores ya tipados.
        # y Compongo el diccionario con la listaKeys y lst_valores_tipados.
        ...
        #                         for j, valorTipado in enumerate(lst_valores_tipados) 
        #                             if i==j
        # }
        # print(diccionarioRetorno)        
        # _______________________
        diccionarioRetorno = dict(zip(listaKeys, lst_valores_tipados))
        # print(diccionarioRetorno)
        pass
    ...
    def __setValoresByDef(tipo, lista):
        ...
        if tipo is int:
            ...
            lista.append('a@a') #Valor por defecto de bool(es engañoso)
        elif tipo == 'dni':
            lista.append('') #Valor por defecto de bool(es engañoso)
            # lista.append('00000000A') #Valor por defecto de bool(es engañoso)
        elif tipo == 'ipv4':
            lista.append(None) #Valor por defecto de bool(es engañoso)
        elif tipo == 'ipv6':
            lista.append(None) #Valor por defecto de bool(es engañoso)
        elif tipo == 'route':
            lista.append('') #Valor por defecto de bool(es engañoso)
    ...
    # *******************************************
    # Igual que byDef() pero obliga (permiteNulo=True) a introducir el dato bueno DESDE EL TECLADO
    ...
    def byTcld(listaStrKeys, listaDef=None, msgIntro='Intro', permiteNulo=False):
        """          
        ...
        if listaDef:       
            ...
            if listTOdict_byTcld.__validaListasEntrada( listaKeys = listaStrKeys, listaDef=listaDef ) == False:
                return None
            listaDef = listTOdict_byTcld.__igualarListas( listaKeys = listaStrKeys, listaDef = listaDef )
            # _________________
            # CREA EL DICCIONARIO de retorno Generando un patron. KEY(lst_keys):VALUE(valor devuelto de _introByTcld)
            # Para Ejecutar una funcion(__introByTcld). 
            dictRetorno = {
                strKey:re.sub(
                    pattern=patron, 
    ...
    def __introByTcld(strValor, listaDefinicion, options=None):
        ...
        [listaDefinicion](list) pasa siempre listaDef =>[(int, False), (int, False)] lista de tuplas tipo, b_Permite_Nulo
        [options](dict)= { 'msgIntro':msgIntro , 'permiteNulo': permiteNulo , 'capital':esCapital } las opciones que se pasan 
        Return: 
        ejemplo:        
        ...
        while True:
            ...
            try:
                if retorno == '' and listaDefinicion[PERMITENULL] == True:
                    """ Lo hago retornar con este valor a la funcion para que trate los tipos """
                    break
                    ...
                    try:
                        if listaDefinicion[TIPO] is bool:
                            ...
                            if retorno == None:
                                continue
                        elif listaDefinicion[TIPO] is list:
                            retorno = listTOdict_byTcld.__tratarListas(retorno)
                            if not retorno: 
                                continue
                            else:
    ...
    def __tratarListas(str_to_lista:str):
        """ Convierte un string separado por , y retorna una lista con los elementos """
